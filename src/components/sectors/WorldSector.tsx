...
import { Globe, ExternalLink, Radio } from 'lucide-react';
...
export default function WorldSector({ onLog }: Props) {
  ...
  useEffect(() => {
    ...
    const drawGlobe = () => {
      ...
      grad.addColorStop(0.6, 'rgba(0,15,35,0.95)');
      ...
      NEWS_HOTSPOTS.forEach((hs: Hotspot) => {
        ...
        if (p.z < 0) return;
        const pulse = 0.5 + 0.5 * Math.sin(time * 2 + hs.lng);
        const size = 3 + hs.intensity * 4;
        ctx.beginPath();
        ctx.arc(p.x, p.y, size * pulse, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(255,${30 + Math.floor(pulse * 50)},30,${0.6 + pulse * 0.4})`;
        ctx.fill();
        ctx.beginPath();
      ...
      atmosphereGrad.addColorStop(0, 'rgba(0,210,255,0.08)');
    ...
    <div className="flex h-full gap-4 p-4">
          ...
          {loading ? (
            ...
            news.map(item => (
              <motion.div
                ...
                style={{
                  background: item.priority ? 'rgba(255,30,30,0.08)' : 'rgba(255,255,255,0.04)',
                  border: `1px solid ${item.priority ? 'rgba(255,30,30,0.3)' : 'rgba(0,210,255,0.1)'}`,
                }}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selected?.id === item.id && (
                      <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }}>
                        <p className="text-xs mt-1.5 leading-relaxed" style={{ color: 'rgba(150,180,210,0.7)' }}>
                          {item.summary}
                        </p>
      ...
      <div className="w-72 shrink-0">
        ...
        <div className="mt-3 grid grid-cols-2 gap-1.5">
          {NEWS_HOTSPOTS.slice(0, 6).map(hs => (
            ...
            >
              ...
              <span className="text-xs font-mono" style={{ color: 'rgba(200,220,240,0.7)' }}>{hs.label}</span>
            </div>
          ))}
        </div>
      </div>
